    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="t_container">
            <div class="card shadow-sm ">
                ...
                <form id="save_form" method="post" action="{{route('admin.subscriptions.update_transportation',$record->id)}}" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <input type="date" name="moving_day" id="moving_day" class="form-control mb-2"  value="{{old('moving_day',$record->moving_day)}}" required autocomplete/>
                            </div>

                            <div class="col-md-4">
                                <label class="required form-label">{{trans('sub.trip_time')}}</label>
                                <input type="time" name="trip_time" id="trip_time" value="{{old('trip_time',$record->trip_time)}}" class="form-control mb-2" placeholder="{{trans('sub.trip_time')}}"/>
                            </div>

                            <div class="col-md-4">
                                <label class="required form-label">{{trans('sub.moving_time')}}</label>
                                <input type="time" name="moving_time" id="moving_time" value="{{old('moving_time',$record->moving_time)}}" class="form-control mb-2" placeholder="{{trans('sub.moving_time')}}"/>
                            </div>
...
--}}
                            <div class="col-md-4">
                                ...
                                <input type="number" name="persons_number" id="persons_number" value="{{old('persons_number',$record->persons_number)}}" class="form-control mb-2" placeholder="{{trans('sub.persons_number')}}"/>
                            </div>
