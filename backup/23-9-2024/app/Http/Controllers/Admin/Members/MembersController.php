...
//        dd($one_data);
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            $member_sub = MembersSubscriptions::where('member_id', $id)->first();


            if ($member_sub) {
                MembersGoals::where('member_id', $id)->delete();
                AdditionalMemberSubscriptions::where('member_subscription_id', $member_sub->id)->delete();
            ...
            toastr()->addSuccess(trans('forms.success'));
            return response()->json(['message' => 'Image and member deleted successfully.'], 200);
        } catch (\Exception $e) {
            dd($e);
            DB::rollBack();
