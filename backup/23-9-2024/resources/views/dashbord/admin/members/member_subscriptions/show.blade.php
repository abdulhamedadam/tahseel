    ...
    <div class="col-md-12">
        <table class="table table-bordered hl white-border" style="table-layout: fixed;">
            ...
            <tr>
                ...
                <td style=""> {{$all_data->discount}}({{trans('members_subscription.discount_lable')}}
                    )</td>

                <th style="">{{trans('members.package_duration')}}</th>
                <td style="">{{$all_data->main_subscriptions->duration}}({{trans('members_subscription.duration_lable')}}
                    )</td>
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
            ...
            @foreach($all_data->additional_subscriptions as $item)
                <tr>
                    ...
                    <td class="text-center"> {{$item->special_subscriptions->price}} ({{trans('members_subscription.price_lable')}})</td>
                    <td class="text-center"> {{$item->trainer->user_name}}</td>
                    <td class="text-center"> {{$item->discount}} ({{trans('members_subscription.discount_lable')}}
                        )</td>
