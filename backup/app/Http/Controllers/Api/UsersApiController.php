...
//       dd($request->all());
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($user) {
                $user = new UserResource($user);
                return $this->responseApi($user, 'تم الحفظ بنجاح', 201);
            } else {
                return $this->responseApi(null, 'لم يتم الحفظ', 400);
            }
        } catch (\Exception $e) {
...
//                $this->sendCode($request['phone'],$randomCode);
                return $this->responseApi($user, $uniqueCode, 200);
    ...
    public function update_password(Request $request)
    {
        ...
        try {
            ...
            if ($user) {
                return $this->responseApi($user, 'تم الحفظ بنجاح', 202);
            } else {
                return $this->responseApi(null, 'لم يتم الحفظ', 422);
            }
        } catch (\Exception $e) {
            return $this->responseApiError($e->getMessage(), 500);
